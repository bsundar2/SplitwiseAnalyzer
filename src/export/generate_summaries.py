...
def fetch_transactions_for_analysis(year: int) -> pd.DataFrame:
    ...
    df = df[~df["is_deleted"]]
    df = df[~((df["description"].str.lower() == "payment") & (df["category"] == "General"))]
...
def generate_monthly_summary(df: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    monthly = df.groupby("year_month").agg({
        "my_net": ["sum", "mean", "count"],
        "my_paid": "sum",
        "my_owed": "sum"
    }).reset_index()
    
    # Flatten column names
    monthly.columns = ["Month", "Total Spent (Net)", "Avg Transaction", "Transaction Count", 
    ...
    numeric_cols = ["Total Spent (Net)", "Avg Transaction", "Total Paid", "Total Owed", 
                    "Cumulative Spending", "MoM Change"]
...
def generate_category_breakdown(df: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    # Flatten column names
    category.columns = ["Category", "Total Spent", "Avg Transaction", "Transaction Count"]
...
def generate_monthly_trends(df: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    monthly["3-Month Avg"] = monthly["Total Spent"].rolling(window=3, min_periods=1).mean()
  ...
  python src/export/generate_summaries.py --year 2026 --dry-run
    ...
    for _, row in monthly_summary.iterrows():
        ...
        if existing:
            ...
            if (
                abs(existing["total_spent_net"] - row["Total Spent (Net)"]) > 0.01 or
                abs(existing["avg_transaction"] - row["Avg Transaction"]) > 0.01 or
                existing["transaction_count"] != row["Transaction Count"] or
                abs(existing["total_paid"] - row["Total Paid"]) > 0.01 or
                abs(existing["total_owed"] - row["Total Owed"]) > 0.01 or
                abs(existing["cumulative_spending"] - row["Cumulative Spending"]) > 0.01 or
                abs(existing["mom_change"] - row["MoM Change"]) > 0.01
            ):
                data_changed = True
        else:
            # New month
        ...
        if data_changed:
            ...
            all_months = worksheet.get_col(1, include_tailing_empty=False)[1:]  # Skip header
            
            if year_month in all_months:
                # Update existing row
                row_idx = all_months.index(year_month) + 2  # +1 for 0-index, +1 for header
                ...
                # Append new row
                row_values = [row[col] for col in monthly_summary.columns]
                worksheet.append_table(values=[row_values], start="A1", end=None, dimension="ROWS")
                appended_count += 1
    ...
    if updated_count > 0 or appended_count > 0:
        ...
        print(f"✓ {WORKSHEET_MONTHLY_SUMMARY}: No changes needed ({unchanged_count} unchanged)")
