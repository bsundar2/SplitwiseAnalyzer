...
class DatabaseManager:
    ...
    def get_transaction_by_splitwise_id(self, splitwise_id: int) -> Optional[Transaction]:
        """Retrieve transaction by Splitwise expense ID."""
        conn = self.get_connection()
        cursor = conn.cursor()
        
        cursor.execute("SELECT * FROM transactions WHERE splitwise_id = ?", (splitwise_id,))
        row = cursor.fetchone()
        conn.close()
    ...
    def get_unwritten_transactions(self, year: Optional[int] = None) -> List[Transaction]:
    ...
    def find_potential_duplicates(
        ...
        cursor.execute(query, [merchant, date, tolerance_days, amount, amount_tolerance])
        rows = cursor.fetchall()
        conn.close()
    ...
    def get_transactions_by_source(self, source: str) -> List[Transaction]:
        ...
        cursor.execute(
            "SELECT * FROM transactions WHERE source = ? ORDER BY date",
            (source,)
        )
        rows = cursor.fetchall()
        conn.close()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        cursor.execute("SELECT written_to_sheet, COUNT(*) FROM transactions GROUP BY written_to_sheet")
        ...
        cursor.execute("SELECT COUNT(*) FROM transactions WHERE splitwise_id IS NOT NULL")
        stats['in_splitwise'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM transactions WHERE splitwise_deleted_at IS NOT NULL")
