...
```
┌─────────────────────┐
│   Run Pipeline      │  Parses statements, extracts merchants
└──────────┬──────────┘
           │
           ▼
┌────────────────────────────┐
│ merchant_names_for_review  │  ← You have 385 transactions
│         .csv               │    266 unique merchants
└──────────┬─────────────────┘
           │
           ▼
┌─────────────────────┐
│  Review Tool        │  Interactive CLI to approve/correct
│  (you use this)     │  • [a] approve  • [c] correct
└──────────┬──────────┘  • [s] skip     • [q] quit
           │
           ▼
┌──────────────────────────┐
│ merchant_review_feedback │  Saves your corrections
│        .json             │
└──────────┬───────────────┘
           │
           ▼
┌─────────────────────┐
│  Apply Feedback     │  Updates config with corrections
└──────────┬──────────┘
           │
           ▼
┌────────────────────────────────┐
│ merchant_category_lookup.json  │  ← Updated configuration
└────────────────────────────────┘
           │
           ▼
┌─────────────────────┐
│   Run Pipeline      │  Uses updated config = better results!
└─────────────────────┘
```
...
Now the pipeline uses your corrections! 🎉

## 📊 Current Status

**Your Review File:**
- ✅ File exists: `data/processed/merchant_names_for_review.csv`
- 📊 **385 total transactions**
- 🏪 **266 unique merchants**
- ✏️ **0 reviewed so far** (ready to start!)

**Sample Merchants Waiting for Review:**
1. Wow Auto Detailers - $750.00 (Transportation/Car)
2. Google Fi - $97.01 (Uncategorized/General) ← needs category correction
...
- ✅ "Starbucks" not "STARBUCKS" or "Starbucks Coffee #1234"
- ✅ "Target" not "Target Corporation" or "Target - San Francisco"
- ✅ "Uber" not "Uber Technologies" or "Uber Trip"
...
1. **Review** → 2. **Apply** → 3. **Run Pipeline** → 4. **See Improvements** → (repeat)
