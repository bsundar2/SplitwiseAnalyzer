...
1. **Generate Review Data** - Run the pipeline to create `merchant_names_for_review.csv`
...
| File | Purpose |
|------|---------|
| `data/processed/merchant_names_for_review.csv` | Generated by pipeline, contains extracted merchants for review |
...
First, run the pipeline which will automatically generate a review file for any new merchants:

```bash
# Set PYTHONPATH for module imports
export PYTHONPATH=/home/balaji94/PycharmProjects/SplitwiseImporter

# Run the pipeline on your statement
python src/pipeline.py --statement data/raw/your_statement.csv
```

This creates `data/processed/merchant_names_for_review.csv` with unique merchant entries.
